...
import POSHeader from "@/components/pos/SalesInterface/POSHeader"
import OrderActions from "@/components/pos/SalesInterface/OrderActions"

// Import types and utilities
import type { Product, OrderItem, HeldOrder, CustomerType, DiscountType } from "@/lib/types"
...
export default function SalesInterface({ userId }: SalesInterfaceProps) {
  ...
  const mapOrderTypeToCustomerType = (type: string): Exclude<CustomerType, "pilih"> => {
  ...
  const fetchHeldOrdersFromBackend = async (page = 1) => {
    try {
      ...
      const localOnlyOrders = localOrders.filter((o: HeldOrder) => isNaN(Number(o.id)))
  ...
  const handleCheckout = async () => {
    const finalPaymentMethod: "cash" | "qris" | "transfer" =
      paymentMethod && ["cash", "qris", "transfer"].includes(paymentMethod) ? paymentMethod : "cash"
    if (currentOrder.length === 0) return
      ...
      if (recalledOrderId) {
        ...
        if (!isNaN(Number(recalledOrderId))) {
          ...
          try {
            ...
            console.error("Failed to delete recalled order from backend", deleteErr)
          }
        }

        // Remove from local state
        setHeldOrders((prev) => prev.filter((order) => order.id !== recalledOrderId))
    ...
    <Suspense fallback={<div>Loading...</div>}>
      <div className="flex flex-col md:flex-row h-full bg-background">
        ...
        <div className="flex-1 overflow-auto sm:p-0 lg:p-4">
          ...
          {/* Categories tabs with product grid */}
        ...
        <div className="w-full sm:w-[200px] md:w-[300px] lg:w-[300px] border-t md:border-t-0 md:border-l bg-card">
          <OrderActions
            ...
            setIsHeldOrdersDialogOpen={setIsHeldOrdersDialogOpen}
            heldOrdersCount={heldOrdersCount}
          />

          <div className="p-4 border-b">{/* Customer type and discount selection moved to payment section */}</div>
          ...
          <OrderSummary
            ...
            onUpdateQuantity={(id, newQuantity) => {
              if (newQuantity <= 0) {
                ...
                const updatedOrder = currentOrder.filter((item) => item.product.id !== id)
                ...
                const updatedOrder = currentOrder.map((item) =>
                  item.product.id === id
                    ? {
                        ...
                        subtotal: calculateSubtotal(item.product.price, newQuantity, 0),
                      }
                    : item,
                )
                setCurrentOrder(updatedOrder)
            ...
            discountInfo={discountValue ? { type: discountType, value: discountValue } : undefined}
